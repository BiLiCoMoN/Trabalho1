...
/* Inicializa o programa e carrega os dados do arquivo */
void inicializar() {
    // Inicializa todas as posições do vetorPrincipal como NULL
    for (int i = 0; i < TAM; i++) {
    ...
    if (arquivo != NULL) {
        ...
        while (fgets(linha, sizeof(linha), arquivo) != NULL) {
            ...
            int posicaoLinha, tamLinha;
            if (token != NULL) {
                posicaoLinha = atoi(token);
                token = strtok(NULL, " ");
                if (token != NULL) {
                    tamLinha = atoi(token);
                    if (ehPosicaoValida(posicaoLinha) == SUCESSO && tamLinha >= 0) {
                        criarEstruturaAuxiliar(posicaoLinha, tamLinha);
                        while ((token = strtok(NULL, " ")) != NULL) {
                            int valor = atoi(token);
                            inserirNumeroEmEstrutura(posicaoLinha, valor);
                        }
                    }
                }
            }
...
/* Finaliza o programa e salva os dados no arquivo */
void finalizar() {
    FILE *arquivo = fopen("Dados.txt", "w");
    if (arquivo != NULL) {
        for (int i = 0; i < TAM; i++) {
            if (vetorPrincipal[i] != NULL) {
                fprintf(arquivo, "%d %d", i + 1, vetorPrincipal[i]->tamanho);
                for (int j = 0; j < vetorPrincipal[i]->usados; j++) {
                    fprintf(arquivo, " %d", vetorPrincipal[i]->numeros[j]);
                }
                fprintf(arquivo, "\n");
            }
        }
